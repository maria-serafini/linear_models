...
``` r
fit_interactions <-
  lm(price ~ stars * borough + room_type * borough, data = nyc_airbnb) # allow effect of stars to differ from one borough to the next 
...
nyc_airbnb |> 
  ...
  lm(price ~ stars + room_type, data = _) |> # b/c data is not first argument in lm(), have to tell pipe where to put df from last function into the next one
  broom::tidy()
```
...
Create a list of dataframs, and iterate to fit the model each time.
...
``` r
# look and see what's happening borough to borough
```
